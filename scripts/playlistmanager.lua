...
--END OF SAMPLE REPLACE ]=====]
  ...
  --only when file load ( not start state.)
  show_playlist_on_fileload2 = 2,
...
function playfile()
  ...
  if settings.show_playlist_on_fileload2 ~= 2 then
    --mp.add_timeout(1,remove_keybinds)
    remove_keybinds()
  end
end
...
function add_keybinds()
  ...
  bind_keys(settings.key_playfile2, 'playfile', playfile)
...
function remove_keybinds()
  ...
  if settings.dynamic_binds then
    ...
    unbind_keys(settings.key_playfile2, 'playfile')
...
mp.add_key_binding("+", "showplaylist", toggle_playlist)
